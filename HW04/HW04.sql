...
SELECT 
    s.`shopname` AS shop_name
FROM shops s
WHERE id = (SELECT shops_id FROM cats c WHERE c.`name` = 'Murzik');
...
SELECT 
    ...
    s.shopname
FROM shops s
LEFT JOIN cats c ON c.shops_id = s.id AND (c.name = 'Murzik' OR c.name = 'Zuza')
WHERE c.name IS NULL;

-- Вывести название и цену для всех анализов, которые продавались 5 февраля 2020 и всю следующую неделю.

SELECT 
	a.an_name as analysis_name,
    a.an_price as analysis_price,
    o.ord_datetime as analysis_date
FROM analysis a
